...
class ChooseThemeActivity
    : BaseMVVMPActivity<ActivityChooseThemeBinding, ChooseThemeActivityViewModel>() {

    override fun getViewModel() = mViewModelProvider[ChooseThemeActivityViewModel::class.java]
    ...
    private fun setRadioGroupListener() {
        radio_group_theme.setOnCheckedChangeListener { group, checkedId ->
            mViewModel.saveCurrentTheme(applicationContext, getSelectedThemeName(checkedId))
            recreate()
        }
    }
    ...
    private fun setThemeRadioButtonCheck() {
        with(ThemeConstantUtils) {
            when (mViewModel.getCurrentBaseThemeName(WeakReference(applicationContext))) {
    ...
    private fun getSelectedThemeName(radioButtonId: Int): String {
