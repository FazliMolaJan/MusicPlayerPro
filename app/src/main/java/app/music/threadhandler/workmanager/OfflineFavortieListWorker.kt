...
class OfflineFavortieListWorker(var context: Context, workerParams: WorkerParameters)
    ...
    override fun doWork(): Result {
        return try {
            ...
            if (getAllOfflineSongResult) {
                InformationLogUtils.logFavoriteThreadStart(TAG)
                LoadMusicUtil.getFavoriteList(WeakReference<Context>(context))
                Result.success()
            } else {
                Result.failure()
            }
