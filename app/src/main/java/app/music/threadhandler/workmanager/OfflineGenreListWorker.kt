...
class OfflineGenreListWorker(private var context: Context, workerParams: WorkerParameters)
    ...
    override fun doWork(): Result {
        return try {
            ...
            if (getAllOfflineSongResult) {
                InformationLogUtils.logGenreThreadStart(TAG)
                LoadMusicUtil.getGenre()
                SortMethodUtils.sortGenreList(context)
                Result.success()
            } else {
                Result.failure()
            }
